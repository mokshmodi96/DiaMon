...
public class DocDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.viewdoc_detail);
        String cname= getIntent().getStringExtra("d_name");
        String cmobile= getIntent().getStringExtra("d_mobile");
        String cemail= getIntent().getStringExtra("d_email");
        final String did= getIntent().getStringExtra("d_id");
        String cpic= getIntent().getStringExtra("d_pic");
        sharedPreferences = getSharedPreferences("UserDetails", Context.MODE_PRIVATE);
        final String user_id=sharedPreferences.getString("u_id", "");
        ...
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(DocDetailActivity.this);
        rv_subcrip.setLayoutManager(linearLayoutManager);
        rv_subcrip.setHasFixedSize(true);
        RecyclerView.ItemDecoration dividerItemDecoration = new DividerItemDecoration(DocDetailActivity.this);
        ...
        Glide.with(DocDetailActivity.this)
                .load(cpic)
        ...
        bt_addsub.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                subscriptiondialog(did,user_id);
            }
        });
//        doc_user_medicine_list(user_id,did);


    }

    private void subscriptiondialog(final String did, final String user_id) {
        final Dialog dialog = new Dialog(DocDetailActivity.this);
        Button bt_chooseimage,bt_addmedicine;
        final EditText ed_desc;
        TextView tv_title;

        dialog.setContentView(R.layout.add_report_dialog);
        ...
        bt_addmedicine.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                {
                   add_subc(did,user_id,imagebase64,m_desc)    ;
                    dialog.dismiss();
                }
                else{

                    Toast.makeText(DocDetailActivity.this,"Please Enter Report Desc",Toast.LENGTH_SHORT).show();
                }
    ...
    private void add_subc(String doc_id, String cid, String imagebase64, String m_desc) {
        ...
        Call<ArrayList<commondata>> user_profile_pic = apiService.add_report1("add_report",doc_id,cid,imagebase64,m_desc);
                ...
                if (response.body() != null) {
                    ...
                    if (login.size() > 0) {
                        if (login.get(0).getStatus().equals("1")) {
                            Toast.makeText(DocDetailActivity.this, "User Report Added Successfully", Toast.LENGTH_LONG).show();

                        }
                        else {
                            Toast.makeText(DocDetailActivity.this, " Error in adding Report!!", Toast.LENGTH_LONG).show();
                        }

                    } else {
                        Toast.makeText(DocDetailActivity.this, "Something went wrong,Please try again later!!", Toast.LENGTH_LONG).show();
                    }
                }
            }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == PICK_IMAGE && resultCode == Activity.RESULT_OK) {
            if (data == null) {
                Toast.makeText(DocDetailActivity.this, "No Image Selected", Toast.LENGTH_LONG).show();
                return;
            }
            try {
        ...
        add_complaint_user.enqueue(new Callback<ArrayList<prescdata>>() {
            ...
            public void onResponse(Call<ArrayList<prescdata>> call, Response<ArrayList<prescdata>> response) {
                ...
                if (response.body() != null) {
                    ...
                    {
                        dapater=new DpreListAdapter(DocDetailActivity.this,postList);
                        rv_subcrip.setAdapter(dapater);
                    }
                    else
                    {
                        Toast.makeText(DocDetailActivity.this,"No Medcine List Found",Toast.LENGTH_SHORT).show();
                    }
                }
            }
